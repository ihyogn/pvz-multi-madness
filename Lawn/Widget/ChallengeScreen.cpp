...
{
	// Runtime bounds check (more robust than assertion)
	// This handles cases where invalid indices are passed (e.g., from GAMEMODE_MULTIPLAYER_ONLINE or GAMEMODE_ADVENTURE)
	static ChallengeDefinition dummyDef = { GameMode::GAMEMODE_UPSELL, 10, ChallengePage::CHALLENGE_PAGE_LIMBO, 4, 3, _S("[UPSELL]"), false };
	
	if (theChallengeMode < 0 || theChallengeMode >= NUM_CHALLENGE_MODES)
	{
		// Invalid index - return safe fallback
		return dummyDef;
	}

	ChallengeDefinition& aDef = gChallengeDefs[theChallengeMode];
	
	// Validate that the challenge mode matches the expected enum value
	// Skip this check if it would fail for valid entries
	GameMode expectedMode = (GameMode)(theChallengeMode + (int)GAMEMODE_SURVIVAL_NORMAL_STAGE_1);
	if (aDef.mChallengeMode != expectedMode && 
	    aDef.mChallengeMode != GameMode::GAMEMODE_UPSELL && 
	    aDef.mChallengeMode != GameMode::GAMEMODE_INTRO)
	{
		// Entry doesn't match expected mode - return safe fallback
		return dummyDef;
	}
