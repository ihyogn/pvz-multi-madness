...
void GameSelector::UpdateTooltip()
{
	...
	if (mHasTrophy)
	{
		...
		if (Rect(50 + BOARD_ADDITIONAL_WIDTH, 325 + BOARD_OFFSET_Y, 85, HAS_ACHIEVEMENTS ? 142 : 225).Contains(aMouseX, aMouseY))
		{
			if (mApp->EarnedGoldTrophy())
			{
	...
	if (mStartingGame)
	{
		...
		if (mStartingGameCounter > 450)
		{
			...
			if (mApp->mPlayerInfo->mNeedsMagicTacoReward && mLevel == 35)
			{
				...
				// PreNewGame will show connection dialog if needed
			...
			// Show network connection dialog before starting adventure
			// Adventure mode now requires multiplayer connection
			mApp->DoNetworkConnectionDialog();
			// PreNewGame will be called after connection is established from the dialog
			return;
		}
