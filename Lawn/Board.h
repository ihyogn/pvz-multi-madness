...
public:
	...
	// Visual-only zombie data for clients (host doesn't use this)
	struct VisualZombie
	{
		...
		bool mIsEating;
		bool mDead;
		VisualZombie() : mZombieID(0), mZombieType(ZOMBIE_NORMAL), mPosX(0), mPosY(0), mRow(0),
			mZombiePhase(PHASE_ZOMBIE_NORMAL), mFrame(0), mIsEating(false), mDead(false) {}
	};
	static const int MAX_VISUAL_ZOMBIES = 200;
	VisualZombie mVisualZombies[MAX_VISUAL_ZOMBIES];
	int mVisualZombieCount;												
...
public:
	...
	void							ProcessNetworkActions(bool theSendZombieSync = true);
	void							UpdateZombieSeedBank();  // Update seed bank when zombie selection changes
