...
bool Board::AreEnemyZombiesOnScreen()
{
	...
	// On clients, also check visual zombies (fake zombies for counting)
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	    mApp->mNetworkManager && !mApp->mNetworkManager->IsHost())
	{
		for (int i = 0; i < mVisualZombieCount; i++)
		{
			if (!mVisualZombies[i].mDead)
			{
				return true;
			}
		}
...
int Board::CountZombiesOnScreen()
{
	...
	// On clients, also count visual zombies (fake zombies for counting)
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	    mApp->mNetworkManager && !mApp->mNetworkManager->IsHost())
	{
		for (int i = 0; i < mVisualZombieCount; i++)
...
void Board::InitLevel()
{
	...
	else if (mApp->IsAdventureMode() && mApp->mPlayerInfo)
	...
	else if (aGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mApp->mNetworkManager &&
	         mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES)
	{
		// Player 2 (zombie player) should start with sun money like I Zombie mode
		mSunMoney = 150;
	}
	...
	{
		// Set up seed bank based on player role
		if (mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES)
		{
			// Player 2 (zombie player) gets zombie seeds based on host selection
			// Populate seed bank from zombie selection
			int packetIndex = 0;
			for (int i = 0; i < NUM_ZOMBIE_TYPES && packetIndex < SEEDBANK_MAX; i++)
			{
				ZombieType zType = (ZombieType)i;
				if (mApp->mNetworkManager->IsZombieSelected(zType))
				{
					SeedType seedType = Challenge::IZombieTypeToSeedType(zType);
					if (seedType != SEED_NONE)
					{
						mSeedBank->mSeedPackets[packetIndex].SetPacketType(seedType);
						packetIndex++;
					}
				}
			}
			
			// Always ensure we have at least some zombies (fallback defaults)
			if (packetIndex == 0)
			{
				mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_ZOMBIE_NORMAL);
				mSeedBank->mSeedPackets[1].SetPacketType(SeedType::SEED_ZOMBIE_TRAFFIC_CONE);
			...
			// Set the actual number of packets
			mSeedBank->mNumPackets = packetIndex;
			mSeedBank->UpdateWidth();
		}
		// Host (Player 1) uses normal plant seeds (handled by default case below)
	}
	
	if (!ChooseSeedsOnCurrentLevel() && !HasConveyorBeltSeedBank() && 
	    (aGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE || !mApp->mNetworkManager || 
	     mApp->mNetworkManager->GetMyRole() != NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES))
	{
		mSeedBank->mNumPackets = GetNumSeedsInBank();
...
{
	GameMode aGameMode = mApp->mGameMode;
	// Disable lawnmowers in multiplayer mode
	if (aGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE ||
		...
		mApp->IsSquirrelLevel() || mApp->IsIZombieLevel() || (StageHasRoof() && mApp->mPlayerInfo && !mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_ROOF_CLEANER]))
		return;
...
void Board::UpdateLevelEndSequence()
{
	...
	if (mBoardFadeOutCounter == 0)
	{
		// In multiplayer, restart the game just like EndLevel() does
		if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
		{
			mApp->EndLevel();
			return;
...
void Board::FadeOutLevel()
{
	...
	// In multiplayer, auto-restart after win/loss
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	    (mApp->mBoardResult == BoardResult::BOARDRESULT_WON || mApp->mBoardResult == BoardResult::BOARDRESULT_LOST))
	{
		mBoardFadeOutCounter = 180;  // 3 seconds at 60 FPS before auto-restart
		return;
	}
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	// In multiplayer mode, check if we should handle I Zombie mode (Player 2 plays zombies)
	if (mApp->IsIZombieLevel() || 
	    (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	     mApp->mNetworkManager && 
	     mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES))
	{
	...
	else if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK)
	{
		...
		// Send plant placement to other players in multiplayer mode
		if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mApp->mNetworkManager && 
		    mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_HOST_PLANTS)
		{
			mApp->mNetworkManager->SendPlantPlaced(aGridX, aGridY, aPlantingSeedType);
		}
	}
	else
	{
...
void Board::MouseDown(int x, int y, int theClickCount)
{
	...
	// Handle hammer player (Player 3) clicks - can click anywhere to strike
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mApp->mNetworkManager &&
	    mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_HAMMER)
	{
		// Hammer player can click anywhere on the board to strike
		// Skip menu/store button clicks
		if (!mMenuButton->IsMouseOver() && (!mStoreButton || !mStoreButton->IsMouseOver()) && 
		    !mFastButton->IsMouseOver() && theClickCount > 0)
		{
			mApp->mNetworkManager->SendHammerStrike(x, y);
			UpdateCursor();
			return;
		}
	}
...
void Board::MouseUp(int x, int y, int theClickCount)
{
	...
	if (CanInteractWithBoardButtons() && theClickCount > 0)
	{
		if (mMenuButton->IsMouseOver() && !mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) && !mApp->GetDialog(Dialogs::DIALOG_LEVEL_COMPLETE))
		{
			...
			// In multiplayer mode, disconnect and return to main menu
			if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
			{
				if (mApp->mNetworkManager)
				{
					mApp->mNetworkManager->Disconnect();
					if (mApp->mNetworkManager->IsHost())
					{
						mApp->mNetworkManager->StopHost();
					}
				}
...
{
	// Don't allow pausing in multiplayer mode - game must continue for network sync
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
		return;
...
{
	// In multiplayer mode, zombies are only spawned by Player 2 (zombie player) via network
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
		return;
...
void Board::UpdateGameObjects()
{
	...
	// In multiplayer, only host updates zombies - clients receive updates via network
	// In multiplayer mode, always update zombies on host regardless of pause state or scene
	bool shouldUpdateZombies = true;
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	    mApp->mNetworkManager && !mApp->mNetworkManager->IsHost())
	{
		shouldUpdateZombies = false;  // Clients don't update zombies locally
	}
	
	// Update zombies: always in multiplayer mode (no pause/scene checks), or if not paused in single player
	// Only skip if we're in main menu (SCENE_MENU) or zombies are dead
	if (shouldUpdateZombies && 
	    (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE || !mPaused))
	{
		Zombie* aZombie = nullptr;
		while (IterateZombies(aZombie))
		{
			// Always update zombies in multiplayer, or if not paused in single player
			// Zombie::Update() will handle death checks internally
			if (!aZombie->mDead)
			{
				aZombie->Update();
			}
	...
	// Clients: Don't update zombies at all - they use visual-only zombies
	// Real zombies are only on the host; clients just display visual data
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	// Send game lost message in multiplayer (host only)
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	    mApp->mNetworkManager && mApp->mNetworkManager->IsHost())
	{
		mApp->mNetworkManager->SendGameLost();
		// Set fade out counter for auto-restart
		mBoardFadeOutCounter = 180;  // 3 seconds at 60 FPS before auto-restart
	}
	...
	else
	{
		...
		if (theZombie)
		{
			// Only call WalkIntoHouse if zombie has valid reanimation (not a fake zombie on client)
			// Fake zombies on clients don't have proper reanimation setup and will crash
			Reanimation* aBodyReanim = mApp->ReanimationTryToGet(theZombie->mBodyReanimID);
			if (aBodyReanim && aBodyReanim->mDefinition)
			{
...
void Board::UpdateSunSpawning()
{
	...
	(mApp->IsIZombieLevel() && !(mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
	                              mApp->mNetworkManager && 
	                              mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES)) ||
...
void Board::Update()
{
	...
	// In multiplayer mode, completely ignore pause state for game logic
	// Only pause UI/cursor in multiplayer, but keep game running
	if (mPaused && mApp->mGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
	{
		mChallenge->Update();
	...
	// In multiplayer, ignore time stop - game must continue for network sync
	if (mTimeStopCounter > 0 && mApp->mGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
		return;
	...
	// Process incoming network messages BEFORE updating game objects so synced positions are applied first
	// (This handles client-side updates from host)
	if (mApp->mNetworkManager && mApp->mNetworkManager->IsConnected())
	{
		ProcessNetworkActions(false);  // false = don't send zombie sync yet (wait until after UpdateGame)
	}
	
	UpdateGame();
	
	// Send zombie sync AFTER updating game objects so host has latest zombie positions
	if (mApp->mNetworkManager && mApp->mNetworkManager->IsConnected())
	{
		ProcessNetworkActions(true);  // true = send zombie sync data from host
	}
...
void Board::DrawGameObjects(Graphics* g)
{
	...
	}
	{
		...
		// Draw visual-only zombies (clients only)
		if (mApp->mNetworkManager && !mApp->mNetworkManager->IsHost())
		{
			for (int i = 0; i < mVisualZombieCount; i++)
			{
				VisualZombie& visualZombie = mVisualZombies[i];
				if (visualZombie.mDead)
					continue;
				
				// Create a temporary zombie for rendering (we'll use DrawZombie which needs a Zombie object)
				// For now, we'll just skip visual zombies and let the host handle all rendering
				// TODO: Implement proper visual zombie rendering
			}
		}
	}
	{
...
void Board::UpdateZombieSeedBank()
{
	if (!mApp->mNetworkManager || !mApp->mNetworkManager->IsConnected())
		return;
	...
	if (!mSeedBank)
		return;  // Seed bank not initialized yet
	
	// Always check for zombie selections and update seed bank
	int packetIndex = 0;
	for (int i = 0; i < NUM_ZOMBIE_TYPES && packetIndex < SEEDBANK_MAX; i++)
	{
		ZombieType zType = (ZombieType)i;
	...
	if (packetIndex > 0)
	{
		mSeedBank->mNumPackets = packetIndex;
		mSeedBank->UpdateWidth();
		mApp->mNetworkManager->ClearZombieSelectionUpdated();
	}
}

void Board::ProcessNetworkActions(bool theSendZombieSync)
{
	if (!mApp->mNetworkManager || !mApp->mNetworkManager->IsConnected())
		return;
	
	NetworkManager* netMgr = mApp->mNetworkManager;
	
	// Update zombie seed bank if selection changed
	UpdateZombieSeedBank();
	
	// Process pending plant placement (from network)
	if (netMgr->HasPendingPlantPlace())
	{
		NetworkManager::PendingPlantPlace& place = netMgr->mPendingPlantPlace;
		if (CanPlantAt(place.mGridX, place.mGridY, place.mSeedType) == PLANTING_OK)
		{
			// Place plant from network (skip sun cost check since it's already validated on sender's side)
			Plant* aPlant = NewPlant(place.mGridX, place.mGridY, place.mSeedType, SeedType::SEED_NONE);
			if (aPlant)
			{
		...
		place.mGridX = -1;
		place.mGridY = -1;
		place.mSeedType = SeedType::SEED_NONE;
	}
	
	// Process pending zombie placement (from network - zombie player sends to host, host broadcasts to all)
	if (netMgr->HasPendingZombiePlace())
	{
		NetworkManager::PendingZombiePlace& place = netMgr->mPendingZombiePlace;
		
		// Only host creates real zombies; clients use visual-only zombies
		if (netMgr->IsHost())
		{
			Zombie* aZombie = AddZombieInRow(place.mZombieType, place.mGridY, 0, true);
			if (aZombie)
			{
			// Override the random position set by ZombieInitialize with exact grid-based position
			// This ensures zombies spawn at the exact same location on all clients
			if (place.mZombieType == ZOMBIE_BUNGEE)
			{
				aZombie->mTargetCol = place.mGridX;
				aZombie->SetRow(place.mGridY);
				aZombie->mPosX = GridToPixelX(place.mGridX, place.mGridY);
				aZombie->mPosY = aZombie->GetPosYBasedOnRow(place.mGridY);
				aZombie->mRenderOrder = MakeRenderOrder(RENDER_LAYER_GRAVE_STONE, place.mGridY, 7);
			}
			else
			{
				// Set exact position from grid coordinates to ensure same spawn location on all clients
				// Use the same calculation as IZombiePlaceZombie for consistency
				aZombie->mPosX = GridToPixelX(place.mGridX, place.mGridY) - 30.0f;
				aZombie->mPosY = aZombie->GetPosYBasedOnRow(place.mGridY);
			}
			
			// Ensure zombie is visible and properly initialized BEFORE setting position
			// This prevents CheckForBoardEdge from triggering on first frame
			aZombie->mDead = false;
			aZombie->mZombieAge = 0;  // Reset age so zombie is fresh
			
			// Update rendering position immediately so zombie is visible at correct location
			aZombie->mX = (int)aZombie->mPosX;
			aZombie->mY = (int)aZombie->mPosY;
			
			// Update attachment position immediately if it exists
			if (aZombie->mAttachmentID != AttachmentID::ATTACHMENTID_NULL)
			{
				AttachmentUpdateAndMove(aZombie->mAttachmentID, aZombie->mPosX, aZombie->mPosY);
			}
			
			// Immediately send full sync data for this newly placed zombie
			// This ensures clients see the zombie immediately with all correct properties
			ZombieID zombieID = ZombieGetID(aZombie);
			NetworkManager::ZombieFullSyncData syncData;
			
			syncData.mZombieID = (unsigned int)zombieID;
			syncData.mZombieType = aZombie->mZombieType;
			syncData.mPosX = aZombie->mPosX;
			syncData.mPosY = aZombie->mPosY;
			syncData.mRow = aZombie->mRow;
			syncData.mBodyHealth = aZombie->mBodyHealth;
			syncData.mHelmHealth = aZombie->mHelmHealth;
			syncData.mShieldHealth = aZombie->mShieldHealth;
			syncData.mZombiePhase = aZombie->mZombiePhase;
			syncData.mFrame = aZombie->mFrame;
			syncData.mIsEating = aZombie->mIsEating;
			syncData.mVelX = aZombie->mVelX;
			syncData.mPhaseCounter = aZombie->mPhaseCounter;
			syncData.mDead = false;
			
			// Get current frame from reanimation if available
			Reanimation* aBodyReanim = mApp->ReanimationTryToGet(aZombie->mBodyReanimID);
			if (aBodyReanim)
			{
				syncData.mFrame = aBodyReanim->mFrameStart;
			}
			
			netMgr->SendZombieFullSyncData(syncData);
			
			mApp->PlayFoley(FOLEY_GROAN);
			}
		}
		// Clients don't create zombies - they use visual-only zombies from sync data
		
		// Clear the pending placement
		place.mGridX = -1;
		place.mGridY = -1;
		place.mZombieType = ZombieType::ZOMBIE_NORMAL;
	}
	
	// Process pending hammer strike (from network - all players process this so everyone sees the kills)
	if (netMgr->HasPendingHammerStrike())
	{
		NetworkManager::PendingHammerStrike& strike = netMgr->mPendingHammerStrike;
		// Kill plants and zombies at the strike location
		int gridX = PixelToGridX(strike.mX, strike.mY);
		int gridY = PixelToGridY(strike.mX, strike.mY);
		
		if (gridX >= 0 && gridX < MAX_GRID_SIZE_X && gridY >= 0 && gridY < MAX_GRID_SIZE_Y)
		{
			// Find and kill ALL plants at this grid position
			Plant* aPlant = nullptr;
			while (IteratePlants(aPlant))
			{
				if (aPlant->mPlantCol == gridX && aPlant->mRow == gridY && !aPlant->mDead)
				{
					aPlant->Die();
				}
			}
			
			// Kill ALL zombies in the area (use a small radius for better hit detection)
			Zombie* aZombie = nullptr;
			while (IterateZombies(aZombie))
			{
				if (aZombie->IsDeadOrDying())
				...
				float zombieX = aZombie->mX + 30.0f;  // Center of zombie
				float zombieY = aZombie->mY + 40.0f;
				float strikeX = strike.mX;
				float strikeY = strike.mY;
				float dist = Distance2D(zombieX, zombieY, strikeX, strikeY);
				
				// Kill if within 60 pixels of strike point
				if (dist < 60.0f)
				{
					aZombie->DieNoLoot();
				}
		...
		// Clear the pending strike
		strike.mX = -1;
		strike.mY = -1;
		strike.mPlayerID = -1;
	}
	
	// Process pending plant death (from network)
	if (netMgr->HasPendingPlantDeath())
	{
		NetworkManager::PendingPlantDeath& death = netMgr->mPendingPlantDeath;
		// Kill all plants at this grid position
		Plant* aPlant = nullptr;
		while (IteratePlants(aPlant))
		{
			if (aPlant->mPlantCol == death.mGridX && aPlant->mRow == death.mGridY && !aPlant->mDead)
			{
				aPlant->Die();
				break;  // Usually only one plant per grid square
			}
		}
		// Clear the pending death
		death.mGridX = -1;
		death.mGridY = -1;
	}
	
	// Process pending zombie death (from network)
	if (netMgr->HasPendingZombieDeath())
	{
		NetworkManager::PendingZombieDeath& death = netMgr->mPendingZombieDeath;
		// Kill zombies at this grid position (find closest zombie in the area)
		...
		while (IterateZombies(aZombie))
		{
			...
			int zombieGridX = PixelToGridX(aZombie->mX + 30, aZombie->mY + 40);
			int zombieGridY = aZombie->mRow;
			
			// Find zombie in the same grid cell
			if (zombieGridX == death.mGridX && zombieGridY == death.mGridY)
			{
				float dist = Distance2D(aZombie->mX, aZombie->mY, GridToPixelX(death.mGridX, death.mGridY), GridToPixelY(death.mGridX, death.mGridY));
				if (dist < closestDist)
				{
					closestDist = dist;
	...
	// Process pending game won (from network)
	if (netMgr->HasPendingGameWon())
	{
		netMgr->mPendingGameWon = false;
		
		// Set board result
		mApp->mBoardResult = BoardResult::BOARDRESULT_WON;
		
		// If level award hasn't been spawned yet, spawn it
		if (!mLevelAwardSpawned)
		{
			// Find a zombie to spawn award from (or spawn at center)
			Zombie* aZombie = nullptr;
			Zombie* awardZombie = nullptr;
			while (IterateZombies(aZombie))
			{
				if (!aZombie->IsDeadOrDying())
				{
			...
			if (awardZombie)
			{
				awardZombie->TrySpawnLevelAward();
			}
			else
			{
				// Spawn award at center if no zombies
				mLevelAwardSpawned = true;
				mApp->mBoardResult = BoardResult::BOARDRESULT_WON;
			}
		}
		
		// Start fade out for auto-restart in multiplayer
		FadeOutLevel();
	}
	
	// Process pending game lost (from network)
	if (netMgr->HasPendingGameLost())
	{
		netMgr->mPendingGameLost = false;
		
		// Set board result
		mApp->mBoardResult = BoardResult::BOARDRESULT_LOST;
		
		// Show game over dialog if not already shown
		if (mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) == nullptr)
		{
			SexyString aGameOverMsg;
			if (mApp->IsIZombieLevel())
			{
	...
	// Process pending game restart (from network)
	if (netMgr->HasPendingGameRestart())
	{
		netMgr->mPendingGameRestart = false;
		
		// Restart the game
		mApp->EndLevel();
	}
	
	// VISUAL-ONLY ZOMBIE SYNC: Clients only store visual data, don't create/update real Zombie objects
	// Host manages all zombie logic; clients just display what host sends
	if (!netMgr->IsHost())
	{
		// On clients: Store visual zombie data only (no real Zombie objects)
		// Track which zombie IDs exist on host to remove visual zombies that no longer exist
		static unsigned int hostZombieIDs[200];
		static int hostZombieIDCount = 0;
		int currentHostZombieIDCount = 0;
		
		for (int i = 0; i < netMgr->mPendingZombieSyncCount; i++)
		{
			NetworkManager::PendingZombieSync& sync = netMgr->mPendingZombieSyncs[i];
			if (sync.mZombieID == 0)
				continue;
			
			// Track this zombie ID as existing on host
			if (currentHostZombieIDCount < 200)
				hostZombieIDs[currentHostZombieIDCount++] = sync.mZombieID;
			
			// Find or create visual zombie entry
			VisualZombie* visualZombie = nullptr;
			for (int j = 0; j < mVisualZombieCount; j++)
			{
				if (mVisualZombies[j].mZombieID == sync.mZombieID)
				{
					visualZombie = &mVisualZombies[j];
					break;
				}
			}
			
			// Create new visual zombie if needed (also create fake zombie object for counting)
			if (!visualZombie && mVisualZombieCount < MAX_VISUAL_ZOMBIES && !sync.mDead)
			{
				visualZombie = &mVisualZombies[mVisualZombieCount++];
				visualZombie->mZombieID = sync.mZombieID;
				
				// Create a fake zombie object for counting and animation purposes
				// This zombie will have proper reanimation so it can be animated from host sync data
				Zombie* fakeZombie = AddZombieInRow(sync.mZombieType, sync.mRow, 0, true);
				if (fakeZombie)
				{
					fakeZombie->mPosX = sync.mPosX;
					fakeZombie->mPosY = sync.mPosY;
					fakeZombie->mX = (int)sync.mPosX;
					fakeZombie->mY = (int)sync.mPosY;
					fakeZombie->SetRow(sync.mRow);
					fakeZombie->mDead = false;
					fakeZombie->mZombieAge = 0;
					fakeZombie->mHasHead = true;  // Important for counting
					fakeZombie->mZombiePhase = sync.mZombiePhase;
					fakeZombie->mIsEating = sync.mIsEating;
					
					// Update reanimation frame from sync data
					Reanimation* aBodyReanim = mApp->ReanimationTryToGet(fakeZombie->mBodyReanimID);
					if (aBodyReanim)
					{
						aBodyReanim->mFrameStart = sync.mFrame;
						aBodyReanim->mAnimTime = sync.mFrame;
						// Update reanimation position
						AttachmentUpdateAndMove(fakeZombie->mAttachmentID, fakeZombie->mPosX, fakeZombie->mPosY);
					}
				}
			}
			
			// Update visual zombie data and corresponding fake zombie
			if (visualZombie)
			{
				visualZombie->mZombieType = sync.mZombieType;
				visualZombie->mPosX = sync.mPosX;
				visualZombie->mPosY = sync.mPosY;
				visualZombie->mRow = sync.mRow;
				visualZombie->mZombiePhase = sync.mZombiePhase;
				visualZombie->mFrame = sync.mFrame;
				visualZombie->mIsEating = sync.mIsEating;
				visualZombie->mDead = sync.mDead;
				
				// Update corresponding fake zombie if it exists
				Zombie* fakeZombie = nullptr;
				Zombie* checkZombie = nullptr;
				while (IterateZombies(checkZombie))
				{
					// Match by position and type (since we can't easily store the visual zombie ID)
					if (checkZombie->mZombieType == visualZombie->mZombieType &&
					    checkZombie->mRow == visualZombie->mRow &&
					    abs(checkZombie->mPosX - visualZombie->mPosX) < 50.0f &&
					    abs(checkZombie->mPosY - visualZombie->mPosY) < 50.0f)
					{
						fakeZombie = checkZombie;
						break;
					}
				}
				...
				if (fakeZombie)
				{
					// Update fake zombie position and death state
					fakeZombie->mPosX = visualZombie->mPosX;
					fakeZombie->mPosY = visualZombie->mPosY;
					fakeZombie->mX = (int)visualZombie->mPosX;
					fakeZombie->mY = (int)visualZombie->mPosY;
					fakeZombie->mZombiePhase = visualZombie->mZombiePhase;
					fakeZombie->mIsEating = visualZombie->mIsEating;
					fakeZombie->mDead = visualZombie->mDead;
					
					// Update reanimation frame from host sync data (copy animation from host)
					Reanimation* aBodyReanim = mApp->ReanimationTryToGet(fakeZombie->mBodyReanimID);
					if (aBodyReanim && aBodyReanim->mDefinition)
					{
						// Copy animation frame from host
						aBodyReanim->mFrameStart = visualZombie->mFrame;
						aBodyReanim->mAnimTime = visualZombie->mFrame;
						// Update reanimation position to match zombie position
						AttachmentUpdateAndMove(fakeZombie->mAttachmentID, fakeZombie->mPosX, fakeZombie->mPosY);
					}
					
					if (visualZombie->mDead && !fakeZombie->IsDeadOrDying())
					{
						fakeZombie->DieNoLoot();
					}
				}
			}
			...
			sync.mNeedsCreation = false;
		}
		
		// Remove visual zombies that no longer exist on host (and their fake zombie objects)
		if (currentHostZombieIDCount > 0)
		{
			int writeIdx = 0;
			for (int i = 0; i < mVisualZombieCount; i++)
			{
				bool found = false;
				for (int j = 0; j < currentHostZombieIDCount; j++)
				{
					if (hostZombieIDs[j] == mVisualZombies[i].mZombieID)
					{
						found = true;
						break;
					}
				...
				{
					if (writeIdx != i)
						mVisualZombies[writeIdx] = mVisualZombies[i];
					writeIdx++;
				}
				else
				{
					// Remove corresponding fake zombie
					Zombie* fakeZombie = nullptr;
					Zombie* checkZombie = nullptr;
					while (IterateZombies(checkZombie))
					{
						if (checkZombie->mZombieType == mVisualZombies[i].mZombieType &&
						    checkZombie->mRow == mVisualZombies[i].mRow &&
						    abs(checkZombie->mPosX - mVisualZombies[i].mPosX) < 50.0f &&
						    abs(checkZombie->mPosY - mVisualZombies[i].mPosY) < 50.0f)
						{
							fakeZombie = checkZombie;
							break;
						}
			...
			mVisualZombieCount = writeIdx;
			hostZombieIDCount = currentHostZombieIDCount;
		}
		
		// Clean up unused sync entries
		int writeIdx = 0;
		for (int i = 0; i < netMgr->mPendingZombieSyncCount; i++)
		{
			if (netMgr->mPendingZombieSyncs[i].mZombieID != 0)
			{
				if (writeIdx != i)
					netMgr->mPendingZombieSyncs[writeIdx] = netMgr->mPendingZombieSyncs[i];
				writeIdx++;
			}
		}
		netMgr->mPendingZombieSyncCount = writeIdx;
	}
	else
	{
		// On host: Clean up unused sync entries only
		int writeIdx = 0;
		for (int i = 0; i < netMgr->mPendingZombieSyncCount; i++)
		{
			if (netMgr->mPendingZombieSyncs[i].mZombieID != 0)
	...
	// NEW COMPREHENSIVE SYNC SYSTEM: Send all zombie data using full sync messages
	// This ensures clients always have complete, up-to-date zombie information
	// Only send sync if requested (after UpdateGame) so we have latest positions
	if (theSendZombieSync && netMgr->IsHost())
	{
		Zombie* aZombie = nullptr;
		while (IterateZombies(aZombie))
		{
			// Send full sync for all zombies (alive and recently dead for death sync)
			ZombieID zombieID = ZombieGetID(aZombie);
			...
			// Send full sync data (includes all properties in one message)
			netMgr->SendZombieFullSyncData(syncData);
		}
	}
}
...
int Board::GetNumSeedsInBank()
{
	...
	// Multiplayer mode - Player 2 (zombie client) uses zombie seeds count (handled in InitLevel)
	if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mApp->mNetworkManager &&
	    mApp->mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES)
	{
		// Return a reasonable default - actual count is set in InitLevel based on selection
		return 9;
	}
...
Zombie* Board::GetWinningZombie()
{
	...
	while (IterateZombies(aZombie))
	{
		...
		{
			// On clients, skip fake zombies (they don't have proper reanimation)
			if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
			    mApp->mNetworkManager && !mApp->mNetworkManager->IsHost())
			{
				Reanimation* aBodyReanim = mApp->ReanimationTryToGet(aZombie->mBodyReanimID);
				if (!aBodyReanim || !aBodyReanim->mDefinition)
				{
					continue;  // Skip fake zombies without reanimation
				}
			}
			return aZombie;
		}
