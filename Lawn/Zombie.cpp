...
void Zombie::Update()
{
    ...
    // In multiplayer mode, always update zombies regardless of scene (except main menu)
    // This ensures zombies continue moving even when host tabs out or game is paused
    if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
        mApp->mGameScene != GameScenes::SCENE_MENU)
    {
        doUpdate = true;
    }
    else if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO && mZombieType == ZombieType::ZOMBIE_BOSS)
    {
        doUpdate = true;
    }
...
void Zombie::CheckForBoardEdge()
{
    ...
    {
        // Skip edge check for newly placed zombies (first 10 frames) in I Zombie mode to prevent immediate loss
        // This allows zombies to be placed at any valid grid position without triggering loss
        if (mApp->IsIZombieLevel() && mZombieAge < 10)
        {
            // Don't check edge for newly placed zombies in I Zombie mode
            return;
        }
        
        if (mApp->IsIZombieLevel())
        {
            // In I Zombie mode (including multiplayer), zombies fade out instead of causing loss
            if (mZombieFade == -1)
                mZombieFade = 200;
        }
...
bool Zombie::TrySpawnLevelAward()
{
    ...
    // Send game won message in multiplayer (host only)
    if (mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
        mApp->mNetworkManager && mApp->mNetworkManager->IsHost())
    {
        mApp->mNetworkManager->SendGameWon();
    }
...
{
    // Send zombie death over network in multiplayer mode (host only)
    if (mBoard && IsOnBoard() && mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
        mApp->mNetworkManager && mApp->mNetworkManager->IsHost() && !mDead)
    {
        int gridX = mBoard->PixelToGridX((int)mPosX + 30, (int)mPosY + 40);
        int gridY = mRow;
        mApp->mNetworkManager->SendZombieDied(gridX, gridY);
    }
