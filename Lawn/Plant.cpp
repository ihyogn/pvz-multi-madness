...
void Plant::Die()
{
    // Send plant death over network in multiplayer mode (host only)
    if (IsOnBoard() && mApp->mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && 
        mApp->mNetworkManager && mApp->mNetworkManager->IsHost() && !mDead)
    {
        mApp->mNetworkManager->SendPlantDied(mPlantCol, mRow);
    }
