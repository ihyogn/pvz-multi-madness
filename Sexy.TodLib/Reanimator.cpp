...
bool Reanimation::DrawTrack(Graphics* g, int theTrackIndex, int theRenderGroup, TodTriangleGroup* theTriangleGroup)
{
	...
	// Scale up zombie heads (like mobile port) - make heads 1.5x bigger, except for Dr. Zomboss
	float aHeadScale = 1.0f;
	if (mReanimationType == ReanimationType::REANIM_ZOMBIE)
	{
		const char* aTrackName = mDefinition->mTracks[theTrackIndex].mName;
		// Exclude boss tracks (boss uses REANIM_BOSS type, so this check is extra safety)
		if (strstr(aTrackName, "Boss") == nullptr && strstr(aTrackName, "boss") == nullptr)
		{
			if (strstr(aTrackName, "head") != nullptr || strstr(aTrackName, "Head") != nullptr || 
			    strstr(aTrackName, "neck") != nullptr || strstr(aTrackName, "Neck") != nullptr ||
			    strstr(aTrackName, "hair") != nullptr || strstr(aTrackName, "Hair") != nullptr ||
			    strstr(aTrackName, "tongue") != nullptr || strstr(aTrackName, "Tongue") != nullptr)
			{
				aHeadScale = 1.5f;  // 50% bigger heads
			}
		}
	}
	// Also apply to other zombie reanimation types (like GARGANTUAR, IMP, etc.) but exclude BOSS
	else if (mReanimationType == ReanimationType::REANIM_GARGANTUAR || 
	         mReanimationType == ReanimationType::REANIM_IMP ||
	         mReanimationType == ReanimationType::REANIM_POLEVAULTER ||
	         mReanimationType == ReanimationType::REANIM_ZOMBIE_NEWSPAPER ||
	         mReanimationType == ReanimationType::REANIM_ZOMBIE_FOOTBALL ||
	         mReanimationType == ReanimationType::REANIM_SNORKEL ||
	         mReanimationType == ReanimationType::REANIM_ZOMBIE_ZAMBONI ||
	         mReanimationType == ReanimationType::REANIM_ZOMBIE_DOLPHINRIDER ||
	         mReanimationType == ReanimationType::REANIM_JACKINTHEBOX ||
	         mReanimationType == ReanimationType::REANIM_BALLOON ||
	         mReanimationType == ReanimationType::REANIM_DIGGER ||
	         mReanimationType == ReanimationType::REANIM_POGO ||
	         mReanimationType == ReanimationType::REANIM_YETI ||
	         mReanimationType == ReanimationType::REANIM_BUNGEE ||
	         mReanimationType == ReanimationType::REANIM_LADDER ||
	         mReanimationType == ReanimationType::REANIM_CATAPULT ||
	         mReanimationType == ReanimationType::REANIM_DANCER ||
	         mReanimationType == ReanimationType::REANIM_BACKUP_DANCER ||
	         mReanimationType == ReanimationType::REANIM_BOBSLED)
	{
		const char* aTrackName = mDefinition->mTracks[theTrackIndex].mName;
		if (strstr(aTrackName, "head") != nullptr || strstr(aTrackName, "Head") != nullptr || 
		    strstr(aTrackName, "neck") != nullptr || strstr(aTrackName, "Neck") != nullptr ||
	...
	// Apply head scaling by scaling the transform matrix components directly
	if (aHeadScale != 1.0f)
	{
		// Scale the X and Y scale components of the transform matrix
		aTransformMatrix.m00 *= aHeadScale;
		aTransformMatrix.m01 *= aHeadScale;
		aTransformMatrix.m10 *= aHeadScale;
		aTransformMatrix.m11 *= aHeadScale;
	}
