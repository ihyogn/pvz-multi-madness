...
#include "Lawn/Widget/NetworkConnectionDialog.h"
#include "Lawn/Widget/ZombieSelectionDialog.h"
...
#include "Lawn/System/NetworkManager.h"
...
{
	// Don't auto-pause when tabbing out in multiplayer mode
	if (!mTodCheatKeys && CanPauseNow() && mGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
	{
		DoPauseDialog();
	}
}
...
void LawnApp::PreNewGame(GameMode theGameMode, bool theLookForSavedGame)
{
	...
	// Force multiplayer mode for adventure
	GameMode originalGameMode = theGameMode;
	if (theGameMode == GameMode::GAMEMODE_ADVENTURE)
	{
		// Show connection dialog if not already connected
		if (!mNetworkManager || !mNetworkManager->IsConnected())
		{
			DoNetworkConnectionDialog();
			return;  // Dialog will call PreNewGame again after connection
		}
		theGameMode = GameMode::GAMEMODE_MULTIPLAYER_ONLINE;
	}
	// Also check if we're starting multiplayer and not connected
	else if (theGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
	{
		if (!mNetworkManager || !mNetworkManager->IsConnected())
		{
			DoNetworkConnectionDialog();
	...
	// Check for saved game using original mode (adventure saves work for multiplayer)
	if (theLookForSavedGame)
	{
		// If we converted from adventure to multiplayer, check adventure save file first
		if (originalGameMode == GameMode::GAMEMODE_ADVENTURE && theGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
		{
			std::string aSaveName = GetSavedGameName(originalGameMode, mPlayerInfo->mId);
			
			if (this->FileExists(aSaveName))
			{
				// Don't load adventure saves in multiplayer mode - they're incompatible
				// Just delete the old save and start fresh for multiplayer
				EraseFile(aSaveName);
			}
		}
		// Otherwise use normal save game check (only for non-multiplayer modes)
		if (theGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE && TryLoadGame())
			return;
	}
...
void LawnApp::NewGame()
{
	...
	// Force multiplayer mode and level 1 for multiplayer
	if (mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
	{
		if (mPlayerInfo)
			mPlayerInfo->mLevel = 1;  // Always start at level 1 for multiplayer
	}
...
void LawnApp::DoNetworkConnectionDialog()
{
	KillDialog(Dialogs::DIALOG_NETWORK_CONNECT);
	NetworkConnectionDialog* aDialog = new NetworkConnectionDialog(this);
	CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
	AddDialog(Dialogs::DIALOG_NETWORK_CONNECT, aDialog);
}

void LawnApp::DoNetworkHostDialog()
{
	if (!mNetworkManager)
		return;
...
void LawnApp::DoZombieSelectionDialog()
{
	KillDialog(Dialogs::DIALOG_ZOMBIE_SELECTION);
	ZombieSelectionDialog* aDialog = new ZombieSelectionDialog(this);
	CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
	AddDialog(Dialogs::DIALOG_ZOMBIE_SELECTION, aDialog);
}
...
{
	// Don't pause game in multiplayer mode
	if (mBoard && NeedPauseGame() && mGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE)
	{
		mBoard->Pause(true);
	}
}
...
{
	DoParseCmdLine();
	// Allow multiple instances for multiplayer testing
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	...
	{
		// Skip challenge records for multiplayer/adventure modes
		if (mGameMode != GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mGameMode != GameMode::GAMEMODE_ADVENTURE)
		{
			aUnlockedNewChallenge = !HasBeatenChallenge(mGameMode);
		...
		{
			// For multiplayer/adventure modes, just set to false
			aUnlockedNewChallenge = false;
		}
	}
	else
...
void LawnApp::UpdateFrames()
{
	...
	for (int i = 0; i < aUpdateCount; i++)
	{
		...
		// Update network manager for multiplayer
		if (mNetworkManager)
		{
...
{
	return mGameMode == GameMode::GAMEMODE_ADVENTURE || mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE;
}
...
{
	// In multiplayer mode, Player 2 (zombie client) should be treated as I Zombie mode
	if (mBoard && mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE && mNetworkManager && 
	    mNetworkManager->GetMyRole() == NetworkPlayerRole::NETWORK_ROLE_CLIENT_ZOMBIES)
		return true;
...
{
	// Multiplayer mode and Adventure mode don't have valid challenge indices
	if (mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE || mGameMode == GameMode::GAMEMODE_ADVENTURE)
...
{
	// Multiplayer mode and Adventure mode don't have challenge definitions in the gChallengeDefs array
	// They should not use GetChallengeDefinition, so return a safe fallback
	if (mGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE || mGameMode == GameMode::GAMEMODE_ADVENTURE)
	{
		// Return the UPSELL definition as a safe fallback (it's the last entry in the array)
		// This should rarely be accessed for these modes
		static ChallengeDefinition dummyDef = { GameMode::GAMEMODE_UPSELL, 10, ChallengePage::CHALLENGE_PAGE_LIMBO, 4, 3, _S("[UPSELL]"), false };
		return dummyDef;
	}
	
	// For other modes, calculate the challenge index and validate it
	int challengeIndex = GetCurrentChallengeIndex();
	
	// Validate index is within bounds before calling GetChallengeDefinition
	if (challengeIndex < 0 || challengeIndex >= NUM_CHALLENGE_MODES)
	{
	...
	return GetChallengeDefinition(challengeIndex);
}
...
bool LawnApp::HasBeatenChallenge(GameMode theGameMode)
{
	...
	// Multiplayer mode and Adventure mode don't have challenge records
	if (theGameMode == GameMode::GAMEMODE_MULTIPLAYER_ONLINE || theGameMode == GameMode::GAMEMODE_ADVENTURE)
		return false;
